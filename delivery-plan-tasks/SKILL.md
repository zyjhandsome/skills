...
Follow the shared contract and template (Plan block). Put plan/tasks, decisions, traceability, readiness, validation, gates, and parallel ownership in `stage_payload`; persist `handoff.json` after validation. Generate the skeleton with `../delivery-frame-spec/scripts/delivery_scaffold.py new-handoff delivery-plan-tasks --change-dir <change>`.
