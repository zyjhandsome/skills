...
def validate(data: Any, raw_size: int) -> list[str]:
    ...
    for field, allowed in OPTIONAL_ENUMS.items():
        if field in data and data.get(field) not in allowed:
            errors.append(f"{field} must be one of {sorted(allowed)}")
    ...
    if data.get("parity_topology") == "cross-repo":
        for field in ("baseline_root", "candidate_root"):
            value = data.get(field)
            if not isinstance(value, str) or PLACEHOLDER.match(value.strip()):
                errors.append(f"cross-repo summary requires concrete {field}")
        forbid = data.get("forbid_baseline_mutation")
        if forbid not in {"yes", "true", True}:
            errors.append("cross-repo summary requires forbid_baseline_mutation=yes")
    for field, limit in [
        ("primary_routes", 10),
        ("top_findings", 10),
