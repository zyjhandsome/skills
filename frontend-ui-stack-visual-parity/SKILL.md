...
description: >
  ...
  mostly works but search bars, forms, and tables look wrong. Also use for
  cross-repo A→B parity (baseline on Vue2 source A, candidate on Vue3 host B)
  when aligning migrated pages to A. Use when the user mentions post-upgrade
  style issues, Tailwind Preflight vs Element Plus, tw- prefix coexistence,
  table/form theme drift, visual parity after Element UI→Plus / Vue2→Vue3
  cutover, or 样式对齐 A. Phase A is evidence-only delimitation; Phase B may
  edit CSS/config only after an explicit user go (candidate/host only when
  cross-repo). Does not re-run Vue2→Vue3 path selection or install/upgrade
  dependencies. Produces an independent report, capture manifest, and compact
  summary; strict parity requires a traceable baseline and deterministic
  capture context.
---
...
**Cross-repo (A→B):** caller supplies `baseline_root=A` and `candidate_root=B`
(or 样式对齐 A / 迁入 B). Set `parity_topology=cross-repo`. Phase A may run
against A only to capture baseline; later compare candidate on B. Phase B
`go:visual-fix` may edit **B only** (`forbid_baseline_mutation: yes`).
...
2. Else if `parity_topology=cross-repo`: prefer orchestration dir under host B  
   `<candidate_root>/openspec/change/<change-id>/visual/` (or caller-named path).  
   Do **not** silently write into A's `.ui-stack-visual-parity` when comparing to B.
3. Else candidate: `<project-root>/.ui-stack-visual-parity`

Always restate the absolute path and get `confirm:output-dir` or
`--output-dir <path>` before writing. Until confirmed: read-only; no report files.

When dependency summary has `visual_strategy_hint=needs_choice` (UI replace),
keep `strategy_status=needs_choice` until the human picks「对齐 A」vs「跟 B 设计系统」.
...
| `parity_topology` | `same-repo` (default) / `cross-repo` |
