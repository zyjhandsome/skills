...
1. Capture baseline on **A** (`baseline_root`) with A’s Node/deps; never edit A.
2. Later capture candidate on **B** (`candidate_root`) after port.
3. Prefer A standalone URL over A-inside-B iframe; if iframe-only, mask B shell.
4. Set `parity_topology=cross-repo` and `forbid_baseline_mutation=yes`.
5. Phase B CSS fixes apply only under `candidate_root`.
