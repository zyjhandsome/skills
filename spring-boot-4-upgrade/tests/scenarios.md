...
| Boot 是 GA，但仲裁后新增传递依赖为 RC，扫描零漏洞 | 按实际树逐项核验发布状态，默认选择兼容 GA；无有效例外不能 verified，不用扫描结果替代 GA 证据 |
| 依赖版本是纯数字且可下载，维护方正式发布证据缺失 | 记 unknown 并补证据；私有组件可用维护方正式记录，不凭命名/仓库可用性判断 GA |
