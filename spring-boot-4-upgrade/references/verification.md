...
本文件是阶段门、`verified` / `verified-with-bridges` 谓词和回退的单源。入口只引用，不复述完整条件。[evidence-contract.md](evidence-contract.md) 只核对记录与解析树一致，不替代本文件，也不证明日志真实或代理守权。
...
源为 3.x 时，必须已有对应仓库/模块、3.5 精确解析版本、代码快照、验证范围、命令与通过证据的 3.5 检查点。缺记录、必需检查失败/不可用、记录不对应当前待迁移快照时，不执行 Boot 4 的 `run`，也不手工把主工作树改到 4；先补证据/解决前置条件。原有失败不会自动豁免。可以继续只读诊断和在隔离副本做明确标注的预览，但不能称已跨过验证点。续跑可复用可追溯的合格检查点，不要求重造表格行。显式要求跳过此条件时说明实际缺口，相关结果保持未验证。

结构化预检可用 [证据契约](evidence-contract.md) 的 `--gate boot4`。它只检查记录与当前快照一致，不执行升级，也不授权跳过本门。
...
6. [依赖安全及发布状态检查](dependency-security.md) 的最终验收通过：有可比较的源/目标证据、新增/变更依赖的发布状态核验、最终制品扫描与策略结论，覆盖实际交付范围。High/Critical 逐项列出适用性、修复版本和处置；未知发布状态或非 GA 项按 assessment 处理，不能仅凭 Boot GA 通过。扫描不可用不算零漏洞；兼容桥、漏洞总数下降或测试全绿均不能豁免阻断项。有效例外单独登记，不当作兼容桥、GA 证明或零风险。
