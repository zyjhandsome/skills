...
5. Stage end: emit one complete `delivery-handoff/v1` object (in-progress, blocked, verified, and end states), validate, persist. After verified, state the resolved archive operation (`next_action`); ask before any commit/PR. Backflow returns to frame/plan follow the chain relay rule (`family-contract.md` section 1), including 会话停点覆盖: if the caller required this session to end after the current stage, persist the handoff and stop; otherwise continue in the same session when the host can load the upstream skill directly.
