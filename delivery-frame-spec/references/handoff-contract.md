...
- `family_version` 以 `family-contract.md` 为权威（当前 `delivery-family/1.6`）；major 不受支持时停止自动链式转换。
...
事实重算结论。Visual required 时，`claims_used` 还须落在
`delivery-visual-evidence-template.md` 的外部引用白名单内；白名单字段不能替代 G9。
