...
禁止为了探测而创建 change、修改工件或执行 archive。若探测发现仓库确实未初始化（异常情况），停止并报告，请用户执行 `openspec init` 后继续。
...
- CLI 运行中真实失败（命令报错、schema 读不出）：按 `family-contract.md` 硬前提故障处理——停止 mutation/gate 写入并报告，恢复后继续。
