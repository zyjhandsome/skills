...
| `manual-router4` | Vue Router 4 migration guide | `vue-router` 3 present | `src/router/**`, **plus every `push`/`replace` call site and every `router-view` carrying a `ref`** | `createRouter` + the history implementation that **preserves the entry's current Router 3 `mode`** (`mode` defaults to `'hash'`, so an unset `mode` means `createWebHashHistory`; `createWebHistory` is a separate decision needing a server rewrite rule, never the default); replace `*` catch-all; drop the router-prototype error-swallowing patch and check every name-target navigation for the params its route requires; move any `router-view` `ref` onto the rendered component via `v-slot="{ Component }"` | 404 and guard order; each navigation that the old error swallowing was hiding; **per entry**, that the first `push`/`replace` leaves the `*.html` pathname and query intact; that every parent→page call through a former `router-view` ref still resolves |
