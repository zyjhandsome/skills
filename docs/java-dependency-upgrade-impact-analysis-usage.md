...
> 受众：第一次接触本 Skill 的同学；后半也给要把本 Skill 挂到 delivery-* 的流程维护者。  
...
本 Skill 与 delivery-* 解耦，可单独使用。与 delivery 联用见下文「与 delivery-* 联用」。
...
- 校验：`python java-dependency-upgrade-impact-analysis/scripts/validate_report.py <report.md>`；样例包：`…/fixtures/valid-report.md`  
...
- Owner 内部调整阶梯：`…/references/owner-and-resolution.md`  
...
如何把 **分析-only** 的 Java 升级 Skill 挂到 `delivery-*` 脊柱上。  
...
### 5.3 环境前置（Environment preflight）
